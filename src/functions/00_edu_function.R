...
add_loop_edu_barrier_d <- function(
  ...
  df <- df %>%
    dplyr::mutate(
      ...
      !!barrier_other_d := dplyr::case_when(
        edu_ind_schooling_age_d == 0 ~ NA_character_,  # If edu_ind_schooling_age_d == 0, set to NA_character_
        TRUE ~ as.character(!!rlang::sym(barrier_other))  # Otherwise, keep the original character value
      )
    )
