create_xlsx_education_table <- function(gt_table, wb, sheet_name) {
  ...
  get_spanner_info <- function(gt_table, level, ordered_data) {
    ...
    list(label_spanner = gt_table$`_spanners`$spanner_label[[level]],
  ...
  has_title <- function(gt_table) {
    list(has_title = !is.null(gt_table[["_heading"]][["title"]]),
  ...
  spanner_in_excel <- function(wb, sheet, spanner, row, style) {
    ...
    openxlsx::addStyle(wb = wb, sheet = sheet, rows = row,
  ...
  writeData(wb, sheet_name, x = gt_table$`_boxhead`$column_label, colNames = F,
            startRow = row_start_col_names)
  openxlsx::addStyle(wb, sheet_name, rows = row_start_col_names, cols = 1:ncol(ordered_gt_data),
                     style = column_labels_border)
  ...
  for(i in unique(gt_table$`_row_groups`)) {
    ...
    openxlsx::addStyle(wb = wb, sheet = sheet_name, rows = row_start_stub,
    ...
    writeData(wb, sheet_name, data_to_write, startRow = row_start_stub, colNames = F,
              borders = "surrounding",
              borderColour ="#D3D3D3")
